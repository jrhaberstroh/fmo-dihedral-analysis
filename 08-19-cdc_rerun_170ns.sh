...
for chromo in 367 368 369 370 371 372 373; do
    for filenum in {1..525}; do
        CHROMO=$chromo
        FNAMEBASE=md500-830203
        FNAME=${FNAMEBASE}-R${filenum}
        
        # Specify the major folders
        SRCDIR=$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )
        TOPOLOGY=$HOME/data/2015-09-FMO_conf/
        TRAJECTORIES=$HOME/data/2016-06-fmo500ns/2016-06-fmo500ns/
        ...
        $MDRUN -v -deffnm $OUTDIR/$FNAME -rerun ${TRAJECTORIES}/${FNAME}.xtc \
                >> $OUTDIR/cdc${CHROMO}_170nsX10ps.cdc
